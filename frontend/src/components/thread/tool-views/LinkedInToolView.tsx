...
import { useRealtimeLinkedInAccounts } from '@/hooks/use-realtime-social-accounts';
...
export function LinkedInToolView({
  ...
  const { enabledAccounts: rtEnabled } = useRealtimeLinkedInAccounts(selectedAgentId || 'suna-default');
  if (isStreaming || (!toolResult?.toolOutput && !toolContent)) {
    return <LoadingState title={'Loading LinkedIn data...'} />;
  }

  let accounts: LIAccount[] = [];
  let message = '';
  let output: any = null;
  try {
    ...
    if (typeof raw === 'string') {
      ...
      const obj: any = raw; if ('tool_execution' in obj) output = obj.tool_execution?.result?.output; else if ('output' in obj) output = obj.output; else output = obj;
    }
  ...
  } catch (e) { console.error('LinkedInToolView parse error:', e); }

  // Prefer real-time enabled accounts
  const realtimeAccounts: LIAccount[] = rtEnabled.map((a: any) => ({
    id: a.account_id,
    name: a.account_name,
    email: undefined,
    ...
    <Card className="overflow-hidden border-zinc-200 dark:border-zinc-700 shadow-lg">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-10 h-10 bg-white rounded-lg shadow-md p-1.5">
              <img src="/platforms/linkedin.png" alt="LinkedIn" className="w-full h-full object-contain" />
            </div>
            <div>
      ...
      <CardContent className="p-4">
        {effectiveAccounts.length === 0 ? (
          <LoadingState title={message || 'No LinkedIn accounts connected'} />
        ) : (
                ...
                <Card key={acc.id} className="overflow-hidden bg-card border border-border">
                  <div className="flex items-stretch">
                      ...
                      <div className="shrink-0">
                        {acc.profile_image_url ? (
                          ...
                          <div className="w-16 h-16 rounded-full bg-muted flex items-center justify-center">
                            <img src="/platforms/linkedin.png" alt="LinkedIn" className="h-8 w-8 opacity-60" />
                          </div>
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold truncate">{acc.name}</h4>
                        {acc.email && <p className="text-sm text-muted-foreground">{acc.email}</p>}
                    ...
                    <div className="flex flex-col justify-center gap-2 p-4 bg-transparent">
                      <Button variant="secondary" size="sm" onClick={() => window.open('https://www.linkedin.com', '_blank')}>
                        <ExternalLink className="h-4 w-4 mr-1" /> Open LinkedIn
                      </Button>
                    </div>
                  </div>
                </Card>
